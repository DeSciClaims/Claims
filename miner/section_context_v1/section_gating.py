...
def _parse_decision(raw_output: str, section: SectionRecord, section_summary: SectionSummaryRecord) -> SectionExtractionDecision:
    ...
    return SectionExtractionDecision(
        ...
        expected_claim_types=[str(item).strip() for item in parsed.get("expected_claim_types", []) if str(item).strip()],
        expected_evidence_types=[str(item).strip() for item in parsed.get("expected_evidence_types", []) if str(item).strip()],
        ...
        likely_context_completeness=str(parsed.get("likely_context_completeness", "")).strip(),
    )
