...
def _score_evidence(
    ...
    if not evidence_items:
        ...
        empty_text_count = sum(
            ...
            if isinstance(item, dict) and not str(item.get("summary_text", "") or item.get("evidence_text", "")).strip()
        )
        empty_source_count = sum(
            1
            for item in evidence_items
            if isinstance(item, dict) and not item.get("source_span_ids")
        )
        if empty_text_count:
            score -= min(0.25, empty_text_count * 0.1)
            issue_tags.append("evidence_text_missing")
            missing_elements.append("evidence_text")
            comments.append("One or more evidence items have no evidence text.")
        if empty_source_count:
            score -= min(0.2, empty_source_count * 0.08)
            issue_tags.append("evidence_source_span_missing")
            missing_elements.append("evidence_source_span_ids")
            comments.append("One or more evidence items have no source span IDs.")
...
def _score_gold_alignment(row: dict[str, Any]) -> tuple[float, str, list[str], dict[str, Any]]:
    ...
    for field_name, selected_key in (
        ("subject", "selected_subject"),
        ("predicate", "selected_predicate"),
        ("object", "selected_object"),
    ):
        if str(row.get(selected_key, "")).strip() or str(gold.get(field_name, "")).strip():
            comparisons[field_name] = _text_similarity(row.get(selected_key, ""), gold.get(field_name, ""))
    ...
    if match_score is None or match_score < 0.45:
        ...
        score = min(score, 0.4)
    comment = "Gold alignment score based primarily on claim text, with SPO compatibility fields used only when present."
    if issues:
...
def _coverage_comment(checks: dict[str, bool]) -> str:
    ...
    if not missing:
        return "Claim packet has claim text and source provenance."
